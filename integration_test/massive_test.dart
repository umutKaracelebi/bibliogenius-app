  ...
  group('Massive Testing - Add Book, Copy, and Exchange', () {
    ...
    testWidgets('Add new book, verify details, add copy', (
      ...
      try {
        ...
        // Author - uses Autocomplete, handle missing key by searching for 'Author' text
        Finder authorField = find.byKey(const Key('authorField'));
        if (authorField.evaluate().isEmpty) {
          // Fallback: look for a TextField that is likely the author field
          authorField = find.widgetWithText(TextFormField, 'Author');
          if (authorField.evaluate().isEmpty) {
            authorField = find.widgetWithText(TextFormField, 'Auteur');
          }
        }
        expect(authorField, findsOneWidget, reason: 'Author field not found');
        ...
        final fabAddBtn = find.byKey(const Key('addCopyButton'));
