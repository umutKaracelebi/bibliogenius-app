  ...
  group('Massive Testing - Add Book, Copy, and Exchange', () {
    ...
    testWidgets('Add new book, verify details, add copy', (WidgetTester tester) async {
      ...
      mockServer.listen((HttpRequest request) async {
        print('Mock Server received request: ${request.method} ${request.uri}');
        ...
        } else if (request.uri.path == '/api/peers/incoming' && request.method == 'POST') {
             // Handle P2P Handshake
             final content = await utf8.decoder.bind(request).join();
             print('Mock Peer received handshake: $content');
             request.response.statusCode = 200;
             request.response.write(jsonEncode({'name': mockPeerName, 'url': 'http://localhost:${mockServer.port}'}));
        } else if (request.uri.path == '/api/peers/requests/incoming' && request.method == 'POST') {
      ...
      try {
        ...
        // Open drawer
        final scaffoldFinder = find.byType(ScaffoldWithNav); // Or standard Scaffold
        ...
        final networkNav = find.text('Réseau'); // "Network" in French? Or english.
        ...
        final networkIcon = find.byIcon(Icons.public); // Standard for Network in this app?
        if (networkIcon.evaluate().isNotEmpty) {
           ...
           if (find.text('Network').evaluate().isNotEmpty) {
             ...
             print('WARNING: Could not find Network navigation item. Attempting direct route push if possible?');
        ...
        expect(manualConnectBtn, findsOneWidget, reason: 'Manual Connect button not found');
        await tester.tap(manualConnectBtn);
        await wait(tester);

        await tester.enterText(find.byKey(const Key('manualConnectNameField')), mockPeerName);
        await tester.enterText(find.byKey(const Key('manualConnectUrlField')), 'http://localhost:${mockServer.port}');
        ...
        expect(peerCard, findsOneWidget, reason: '$mockPeerName not found in list after connection');
        
        // Find the "Localhost" text or similar to identify the card? No, "Mock Peer" is unique.
        // Find the ancestor Row (main card layout)
        final cardRow = find.ancestor(of: find.text(mockPeerName), matching: find.byType(Row)).first;
        final browseBtn = find.descendant(of: cardRow, matching: find.text('Browse'));
        
        await tester.ensureVisible(browseBtn);
        expect(browseBtn, findsOneWidget, reason: 'Browse button not found for Mock Peer');
        ...
        expect(mockBookTitle, findsOneWidget, reason: 'Mock book not displayed');
        ...
        if (requestAction.evaluate().isEmpty) requestAction = find.text('Request');
        ...
        if (requestAction.evaluate().isNotEmpty) {
           ...
           if (confirmBtn.evaluate().isNotEmpty) {
             ...
             print('WARNING: Request button not found. Skipping outgoing request verification.');
        }

        print('Step 13: Incoming Request Flow');
        // Simulate incoming request
        final client = HttpClient();
        final req = await client.post('localhost', ApiService.httpPort, '/api/peers/requests/incoming');
        ...
        final requestsNav = find.byIcon(Icons.move_to_inbox); // Icon for requests app drawer?
        ...
        try {
          ...
          print('Warning: Scroll in incoming requests failed or not needed: $e');
        }
        
        expect(bookTitleFinder, findsOneWidget, reason: 'Incoming request not found');
